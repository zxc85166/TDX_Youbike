...
onMounted(() => {
    ...
    var marker = L.marker([store.RoundDetails.Position.PositionLat, store.RoundDetails.Position.PositionLon]).addTo(mymap.value);
    mymap.value.setView([store.RoundDetails.Position.PositionLat, store.RoundDetails.Position.PositionLon], 18);
    marker.bindPopup(
        `<div class="bg-white rounded-lg text-center">
         <div class="text-gray">
           ...
           <p class="mb-2">電話：${store.RoundDetails.Phone}</p>
           <p class="mb-2">地址：${store.RoundDetails.Address}</p>
           <p class="mb-2">營業時間：${store.RoundDetails.OpenTime}</p>
           <p class="mb-2">介紹：${store.RoundDetails.Description}</p>
         </div>
       </div>`
    ).openPopup();
});
