...
<template>
  <header class="bg-yellow w-full h-[92px] relative">
    <div class="flex flex-row py-6 md:mx-6">
      ...
      >
        {{ store.RoundDetails.Name }}
      </div>
      <div
        v-if="store.RoundRestaurant.Name"
        class="mr-5 flex items-center text-lg flex-2"
      >
        {{ store.RoundRestaurant.Name }}
      </div>
    </div>
  </header>
  <div v-if="store.RoundDetails" class="mx-auto grid gap-5 w-4/5 md:w-1/3 my-7">
    <div class="rounded-lg grid place-items-center">
      <img
        :src="store.RoundDetails.Picture.PictureUrl1"
    ...
    <div class="flex items-center">
      ...
      <a
        @click="goRoundDetailsMap()"
        class="bg-yellow rounded-3xl ml-3 min-w-max"
      >
  ...
  <div
    v-if="store.RoundRestaurant"
    class="mx-auto grid gap-5 w-4/5 md:w-1/3 my-7"
  >
    <div class="rounded-lg grid place-items-center">
      <img
        :src="store.RoundRestaurant.Picture.PictureUrl1"
        class="rounded-lg shadow-xl"
      />
    </div>
    <div class="flex items-center">
